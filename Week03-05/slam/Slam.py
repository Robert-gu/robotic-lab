...
class Slam:
    ...
    def update(self, measurements):
        ...
        # ------------------------------------------
        x_hat = np.array(self.robot.get_state()).reshape((3,1))
        markerReshape = []
        for i in range(0,self.markers.shape[1]):
            markerReshape=np.append(markerReshape,self.markers[:,i],axis=0)
        x_hat=np.append(x_hat.reshape(-1,1),markerReshape.reshape(-1,1),axis=0)
